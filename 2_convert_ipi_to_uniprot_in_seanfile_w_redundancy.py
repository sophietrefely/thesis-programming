...
# [<gene_name>, <[uniprot a, uniprot b]>, <some_other_data>, ... <lots of numbers>]

path_to_sean_list_converted_redundant = 'data/sean_list_converted_redundant.txt'
sean_list_converted_redundant = open(path_to_sean_list_converted_redundant, 'w') # w makes it ready to write the file
...
for item in sean_list:
    ...
    if sean_ipi in conversion_dict:
        ...
        uniprot_str = str(uniprot)
        item[1] = uniprot_str #convert ipi to uniprots
        ...
        sean_list_converted_redundant.write(item_to_write)
    else:
        print(sean_ipi) #this gives the ipis that were not assigned to uniprot accession in first run through DAVID.
sean_list_converted_redundant.close() #must close file after writing
