...
sean_list_converted_redundant = open(path_to_sean_list_converted_redundant, 'rU') #'rU' avoids confusion with read and formatting /r
...
for item in sean_converted_list:
    ...
    for sean_uniprot in sean_uniprots: #go through each uniprot in redundant list   
        ...
        if sean_uniprot in fruman_uniprots: #this is list so only uses the first entry - stops repetition as sean_conversion_list is redundant for all possible uniprots
